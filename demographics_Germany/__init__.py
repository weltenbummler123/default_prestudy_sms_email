...
class C(BaseConstants):
    NAME_IN_URL = 'demographics_Germany'
    PLAYERS_PER_GROUP = 2
    NUM_ROUNDS = 1
...
class Demographics(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return {
            'live_question': f"Wohnen Sie in Deutschland?",
            'region_question': "In welchem Bundesland wohnen Sie?",
            'grow_up_question': f"Sind Sie in Deutschland aufgewachsen?",
...
class CommentBox(Page):
    form_model = 'player'
    form_fields = ['comment_box', 'reliability']

    def vars_for_template(player: Player):

        return dict(
            reliability_q='Bitte geben Sie auf der folgenden Skala an, wie zuverlässig Ihre Antworten auf diese Umfrage sind.',
        )
